...
class TestTaskUpdate:
    ...
    @patch(PATCH_CLIENT)
    @patch(PATCH_RESOLVE)
    def test_note_posted_as_task_note(
        self,
        mr: MagicMock,
        mc: MagicMock,
        mg: MagicMock,
    ) -> None:
        """When --note is given, a note is posted to the task."""
        mr.return_value = _CFG
        mg.return_value = _guard_cfg()
        client = MagicMock()
        client.get.return_value = {"id": 100, "container": 42}
        ...
        note_body = client.post.call_args_list[1][1]["body"]
        assert note_body["content"] == "Starting investigation"
        assert note_body["task_id"] == 100
