"""Tests for soar lists — writes: create, update, add-row, remove-row."""
...
class TestListsCreate:
    ...
    def test_create_dry_run(
        ...
        result = CliRunner().invoke(
            cli,
            ["soar", "lists", "create", "--name", "test", "--file", str(f)],
        )
    ...
    def test_create_empty_rejected(
        ...
        """Create with no --file is rejected (SOAR requires at least one row)."""
        mock_res.return_value = soar_cfg()
        client = MagicMock()
        mock_cls.return_value = client
        result = CliRunner().invoke(
        ...
        assert result.exit_code != 0
        assert (
            "at least one row" in result.output.lower()
            or "at least one row" in (result.stderr or "").lower()
        )
        client.post.assert_not_called()

    @patch(PATCH_CLIENT)
    @patch(PATCH_RESOLVE)
    def test_create_empty_file_rejected(
        self,
        mock_res: MagicMock,
        mock_cls: MagicMock,
        tmp_path: Path,
    ) -> None:
        """Create with a --file that has zero rows is rejected."""
        mock_res.return_value = soar_cfg()
        client = MagicMock()
        mock_cls.return_value = client
...
class TestListsAddRow:
    ...
    @patch(PATCH_CLIENT)
    @patch(PATCH_RESOLVE)
    def test_add_row_json_array(
        self,
        mock_res: MagicMock,
        mock_cls: MagicMock,
        mock_g: MagicMock,
    ) -> None:
        """JSON array form parses correctly."""
        mock_res.return_value = soar_cfg()
        mock_g.return_value = True
        client = MagicMock()
        client.get.return_value = {
        ...
        assert result.exit_code == 0
        body = client.post.call_args[1]["body"]
        assert body["content"] == [["c1", "c2"], ["a, with comma", "b"]]

    @patch(PATCH_GUARD)
    @patch(PATCH_CLIENT)
    @patch(PATCH_RESOLVE)
    def test_add_row_bad_json_errors(
        self,
        mock_res: MagicMock,
        mock_cls: MagicMock,
        mock_g: MagicMock,
    ) -> None:
        """Invalid JSON array in --values gives a clear error."""
        mock_res.return_value = soar_cfg()
        mock_g.return_value = True
        client = MagicMock()
        mock_cls.return_value = client
        ...
        assert result.exit_code != 0
        assert (
            "invalid json" in result.output.lower()
            or "invalid json" in (result.stderr or "").lower()
        )
