...
(filter first, then paging; defaults fetch everything).
...
| `rules` | ✅ built | list (--filter, --app, --owner), get (--app, --owner), create (--email-to/--email-subject/--webhook-url conflict with --set), update (same), delete, enable, disable, share, history |
...
|---|---|---|
| MCP server | ✅ built | 5 meta-tools (help/usage/focus/unfocus/run), 129 auto-generated tools, 22 guide resources |
