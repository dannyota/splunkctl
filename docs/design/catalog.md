...
| `soar playbooks` | ✅ built | run (name or id, --container, --scope all/new, --input k=v, --wait, --timeout; name resolution via GET /rest/playbook; guarded mutation; polls to terminal status; pretty-prints JSON message), runs list (--container, --status, --limit), runs get (--blocks for block_results), runs cancel (guarded) |
