...
The server uses **dynamic tool loading**: instead of exposing all 262 tools
...
command. Version 0.12 uses MCP Python SDK 2 and publishes tool-list changes
through MCP 2 subscriptions.
...
## HTTP transport (local mode)

By default the MCP server uses stdio (one agent, one process). For local
clients that need a URL, run it as a streamable-HTTP server:

```bash
splunkctl mcp serve --transport http
```

This starts an HTTP listener on `127.0.0.1:8765`. The host is restricted to
`127.0.0.1`, `localhost`, or `::1`; you can change the loopback address and
port:

```bash
splunkctl mcp serve --transport http --host localhost --port 9000
```
...
# or with a different loopback host and port:
splunkctl mcp install --transport http --host ::1 --port 9000
```

This writes a URL-based entry instead of the stdio command entry. HTTP mode
has no authentication and cannot bind to a non-loopback address.
...
- **HTTP mode** listens only on a loopback address and has no authentication.
